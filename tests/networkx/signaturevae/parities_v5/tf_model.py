...
class CVAE(object):
    ...
    def __init__(self, n_latent=8, n_hidden=50, alpha=0.003):
        self.tvars = tf.trainable_variables()  #获取到所有的训练参数
    ...
    def train(self, data, data_cond, n_epochs=10000, learning_rate=0.005,
        ...
        unreshaped = tf.reshape(self.dec, [-1, input_dim]) #X_hat
        ...
        for i in tqdm(range(n_epochs), desc="Training"):
            ...
            if not i % 1000 and show_progress:
                ...
                mn_v,sd_v = self.sess.run([self.mn,self.sd],feed_dict={X_in: data, self.cond: data_cond, Y: data})
