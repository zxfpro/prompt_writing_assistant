...
prompt_get_infos
"""
    @intels.intellect(IntellectType.inference,"0080",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
...
prompt_base
"""
    @intels.intellect(IntellectType.inference,"0081",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    user_name = "柴东升"
...
素材增量生成
"""
    @intels.intellect(IntellectType.inference,"0082",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    input_ = """
...
传记简介
"""
    @intels.intellect(IntellectType.inference,"0083",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    inputs = """
...
我想你将章节数量控制在8-12个左右
"""
    @intels.intellect(IntellectType.inference,"0084",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    input = """
...
我希望图片素材,保留原本的markdown格式, 如 
 ![出生东北_1](http://39.96.146.47:8000/11.jpg) 
"""
    @intels.intellect(IntellectType.inference,"0085",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        return input_
    inputs_ = """
...
提取地名
"""
    @intels.intellect(IntellectType.inference,"0086",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    user_name = "柴东升"
...
提取人名
"""
    @intels.intellect(IntellectType.inference,"0087",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    user_name = "柴东升"
...
记忆卡片打分
"""
    @intels.intellect(IntellectType.inference,"0088",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    user_name = "柴东升"
...
title 起一个合适的名称
"""
    @intels.intellect(IntellectType.inference,"0089",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
...
"""
    @intels.intellect(IntellectType.inference,"0090",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        input_ = json.loads(extract_(input_,r"json"))
        return input_
    inputs = """
记忆卡片标题: 我的小时候
...
通过文本生成记忆卡片
"""
    @intels.intellect(IntellectType.inference,"0091",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    user_name = "柴东升"
...
"""
    @intels.intellect(IntellectType.inference,"0092",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        input_ = extract_(input_,r"json")
        return input_
    input = """
 # chat_history: 
...
聊天历史生成记忆卡片
"""
    @intels.intellect(IntellectType.inference,"0093",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    user_name = "柴东升"
...
聊天历史生成记忆卡片
"""
    @intels.intellect(IntellectType.inference,"0094",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        input_ = extract_(input_,r"json")
        return input_
...
``` 
"""
    @intels.intellect(IntellectType.inference,"0095",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        input_ = extract_(input_,r"json")
        return input_
...
帮我提取这个人的用户概述, 大概100字
"""
    @intels.intellect(IntellectType.inference,"0096",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
    input = """
...
的格式输出
"""
    @intels.intellect(IntellectType.inference,"0097",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
...
content 的内容为50字左右
"""
    @intels.intellect(IntellectType.summary,"0098",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
...
"""
    @intels.intellect(IntellectType.inference,"0099",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return input_
...
def test_0100():
    ...
    @intels.intellect(IntellectType.inference,"0100",demand = demand,table_name = "llm_prompt")
    def prompts(input_):
        # 可以直接输出, 也可以编写后处理的逻辑 extract_json 等
        return extract_(input_,r"article")
